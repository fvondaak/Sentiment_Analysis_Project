...
def train_and_eval(data_dir=DEFAULT_DATA_DIR, alpha=1.0, C=1.0):
    """Train the basic NB-SVM and return its evaluation results."""
    X_train, X_test, y_train, y_test = load_preprocessed_data(data_dir)
    X_train_weighted, X_test_weighted, log_count_ratio = (
        compute_nb_weighted_features(X_train, X_test, y_train, alpha)
    )
    classifier = train_classifier(X_train_weighted, y_train, C)
...
if __name__ == "__main__":
    ...
    result = train_and_eval(args.data_dir, args.alpha, args.C)
